...
public class UpdateTask extends RequestTask {
    ...
    public void execute(CommandContext context) {
        ...
        try {
            ...
            NetworkHandler.send(new C2SPayload(PayloadTypes.EXECUTE_REQUEST, new TaskContextData(null, 0, id)));
            var serverResponse = NetworkHandler.wait4response();
            if (serverResponse != StatusCodes.OK) {
                if (serverResponse != StatusCodes.TIMEOUT) {
                    outputHandler.println("No element with such id");
                }
                return;
            }
