...
struct image *parse_bmp_image(unsigned char *image_data) {
  ...
  unsigned char *pixel_data = image_data + header->b_offset_bits;
  uint32_t height = header->bi_height;
  uint32_t width = header->bi_width;
  uint8_t offset_bytes = calculate_bmp_offset_bytes(width);
  ...
  for (size_t it = 0; it < height; ++it) {
    for (size_t jt = 0; jt < width; ++jt) {
      size_t vertical_offset =
          it * (width * sizeof(struct pixel) + offset_bytes);
      size_t horizontal_offset = jt * sizeof(struct pixel);
      struct pixel *pixel_ptr =
          (void *)(pixel_data + vertical_offset + horizontal_offset);
...
size_t generate_bmp_image(unsigned char **buffer,
  ...
  uint8_t offset_bytes = calculate_bmp_offset_bytes(input_image->width);
  size_t calculated_size =
      sizeof(struct bmp_header) +
      (input_image->width * sizeof(struct pixel) + offset_bytes) *
          input_image->height;
  *buffer = calloc(calculated_size, sizeof(unsigned char));
  ...
  header->bi_size_image =
      input_image->width * input_image->height * sizeof(struct pixel);
  ...
  const unsigned char *image_data = *buffer + sizeof(struct bmp_header);
  for (size_t it = 0; it < input_image->height; ++it) {
    for (size_t jt = 0; jt < input_image->width; ++jt) {
      size_t vertical_offset =
          it * (input_image->width * sizeof(struct pixel) + offset_bytes);
      size_t horizontal_offset = jt * sizeof(struct pixel);
      struct pixel *pixel_ptr =
          (void *)(image_data + vertical_offset + horizontal_offset);
...
uint8_t calculate_bmp_offset_bytes(const uint32_t width) {
  return width == 0 ? 
         0 : BMP_LINE_MULTIPLICITY -
         width * sizeof(struct pixel) % BMP_LINE_MULTIPLICITY;
}
