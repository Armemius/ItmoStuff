...
enum image_signatures check_image_signature(const unsigned char *image_data,
...
bool check_bmp_signature(const unsigned char *image_data,
                         const size_t data_size) {
...
validate_image_data(const unsigned char *image_data, const size_t data_size,
...
enum image_validation_status validate_bmp_data(const unsigned char *image_data,
                                               const size_t data_size) {
  ...
  if (header->b_offset_bits < BMP_HEADER_SIZE ||
      header->b_offset_bits >= data_size) {
    return IMAGE_VALIDATION_BMP_CORRUPTED_HEADER;
  }
